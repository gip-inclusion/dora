...
from data_inclusion.schema.v1 import ModeAccueil, ReseauPorteur, TypeService
...
from .descriptions import merged_description
...
class Service(ModerationMixin, models.Model):
    ...
    # Dérivée du couple ci-dessus par `save()`, jamais saisie.
...
class FranceTravailOrientableService(models.Model):
    ...
    def clean(self):
        if ReseauPorteur.FRANCE_TRAVAIL.value not in (
            self.structure.reseaux_porteurs or []
        ):
            raise ValidationError(
