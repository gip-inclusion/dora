...
def check_thematiques_exist_in_dora(apps, schema_editor):
    ...
    for thematique_input, thematiques_output in MAPPING_THEMATIQUES.items():
        thematiques = [thematique_input] + thematiques_output
...
def assign_new_thematiques(apps, schema_editor):
    ...
    ServiceSubCategory = apps.get_model("services", "ServiceSubCategory")
    for thematique_input, thematiques_output in MAPPING_THEMATIQUES.items():
        if len(thematiques_output) == 1 and thematiques_output[0] == thematique_input:
            continue
        category_input = ServiceCategory.objects.get(
            value=thematique_input.split("--")[0]
        )
        subcategory_input = ServiceSubCategory.objects.get(value=thematique_input)
        categories_to_add = []
        subcategories_to_add = []
        for thematique_output in thematiques_output:
            if thematique_output == thematique_input:
                continue
            categories_to_add.append(
                ServiceCategory.objects.get(value=thematique_output.split("--")[0])
            )
            subcategories_to_add.append(
                ServiceSubCategory.objects.get(value=thematique_output)
            )
        for service in Service.objects.filter(categories=category_input):
            service.categories.remove(category_input)
            service.categories.add(*categories_to_add)
        for service in Service.objects.filter(subcategories=subcategory_input):
            service.subcategories.remove(subcategory_input)
            service.subcategories.add(*subcategories_to_add)


def remove_old_thematiques(apps, schema_editor):
    ServiceCategory = apps.get_model("services", "ServiceCategory")
    ServiceSubCategory = apps.get_model("services", "ServiceSubCategory")
    for subcategory in ServiceSubCategory.objects.all():
        if subcategory.value not in Thematique:
            if subcategory.service_set.count() == 0:
                logger.info(
                    f"✅ Subcategory {subcategory.value} is obsolete andnot used by any service. Deleting it."
                )
                subcategory.delete()
            else:
                logger.error(
                    f"❌ Subcategory {subcategory.value} is obsolete but used by services. "
                    "Cannot delete it."
                )
    for category in ServiceCategory.objects.all():
        if (
            ServiceSubCategory.objects.filter(value__startswith=category.value).count()
            == 0
        ):
            if category.service_set.count() == 0:
                logger.info(
                    f"✅ Category {category.value} is obsolete and not used by any service. Deleting it."
                )
                category.delete()
            else:
                logger.error(
                    f"❌ Category {category.value} is obsolete but used by services. Cannot delete it."
                )


class Migration(migrations.Migration):
    dependencies = [
        ("services", "0127_add_thematiques_di_v1"),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(
            remove_old_thematiques, reverse_code=migrations.RunPython.noop
        ),
    ]
