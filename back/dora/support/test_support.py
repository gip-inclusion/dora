...
class StructureAdminExportDataTestCase(APITestCase):
    def setUp(self):
    ...
    def test_structures_admin_export_data_basic_fields(self):
        structure = make_structure(department="31")
        ...
        self.assertEqual(data["siret"], structure.siret)
    ...
    def test_structures_admin_export_data_admin_emails(self):
        admin_user = make_user(is_valid=True, is_active=True)
        structure = make_structure(department="31")

        StructureMember.objects.create(
            structure=structure, user=admin_user, is_admin=True
        )
        ...
        self.assertEqual(response.status_code, 200)
        data = response.data[0]
        self.assertIn(admin_user.email, data["admins"])

    def test_structures_admin_export_data_editor_emails(self):
        editor_user = make_user(is_valid=True, is_active=True)
        structure = make_structure(department="31")

        make_service(
            structure=structure, status=ServiceStatus.PUBLISHED, last_editor=editor_user
        )
        ...
        self.assertEqual(response.status_code, 200)
        data = response.data[0]
        self.assertIn(editor_user.email, data["editors"])

    def test_structures_admin_export_data_putative_member_counts(self):
        structure = make_structure(department="31")

        # Un membre potentiel qui doit être validé
        ...
        # Un membre potentiel qui doit être rappelé
        StructurePutativeMember.objects.create(
            structure=structure,
            user=make_user(is_valid=True, is_active=True),
            is_admin=False,
        ...
        self.assertEqual(response.status_code, 200)
        data = response.data[0]
        self.assertEqual(data["num_potential_members_to_validate"], 1)
        self.assertEqual(data["num_potential_members_to_remind"], 1)

    def test_structures_admin_export_data_waiting_structure(self):
        putative_admin = make_user(is_valid=True, is_active=True)
        ...
        response = self.client.post(
            "/structures-admin/export-data/",
            {"slugs": [waiting_structure.slug]},
        )

        self.assertEqual(response.status_code, 200)
        data = response.data[0]
        self.assertIn(putative_admin.email, data["admins_to_remind"])
