<script lang="ts">
  ...
  let onSiteServicesWithCoords = $derived(
    services.filter(
      (service) =>
        ...
        service.distance <= SEARCH_RADIUS_KM &&
        !!service.coordinates
  ...
  function featuresFromServices(
    servicesWithCoords: ServiceWithCoords[]
  ): mlgl.GeoJSONFeature[] {
    return servicesWithCoords.map((service) => ({
      type: "Feature",
      properties: {
        ...service,
      },
  ...
  async function handleMapLoaded() {
    ...
    spiderfy = new Spiderfy(map, {
      forceSpiderifyMinZoom: 14,
      renderMethod: "3D",
      closeOnLeafClick: false,
      onLeafClick: (feature: mlgl.GeoJSONFeature) => {
        if (onServiceClick) {
    ...
    map.addSource("servicesSource", {
      ...
      data: {
        ...
        features: featuresFromServices(onSiteServicesWithCoords),
      },
    });
  ...
  function updateMapContent(services_: ServiceWithCoords[]) {
    if (!map) {
      return;
    }
    ...
    servicesSource.setData({
      type: "FeatureCollection",
      features: featuresFromServices(services_),
    });
  }
