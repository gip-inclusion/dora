...
<div
  class="gap-s16 border-gray-01 px-s24 py-s16 flex flex-col justify-between rounded-md border md:flex-row md:items-center"
>
  ...
  <div class="gap-x-s32 text-f14 flex flex-1 flex-wrap items-center">
    <div class="flex-3 text-center md:text-right">
      {#if isMyself}
        <span class="bg-magenta-10 px-s12 py-s6 rounded-md">Vous</span>
      {/if}
      <slot name="status" />
    </div>
    <div class="flex-2">
