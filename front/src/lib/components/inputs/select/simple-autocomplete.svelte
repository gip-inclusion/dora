...
<script lang="ts">
  ...
  function updateValue(newValue) {
    if (newValue) {
      const newText = multiple ? "" : getLabelForValue(value);
      if (text !== newText) {
        text = newText;
      }
      onChange(newValue);
  ...
  $effect(() => {
    // Au cas où value change directement, on appelle updateValue() pour avoir les effets voulus
    if (value) {
      updateValue(value);
    }
  });
