...
describe("searchDepartment", () => {
  ...
  describe("No match", () => {
    test("unknown department", () => {
      expect(searchDepartment("Bruxelles")).toBeNull();
    });
    test("words in the wrong order", () => {
      // "Loire-Atlantique" contains both words, but not in that order.
      expect(searchDepartment("Atlantique Loire")).toBeNull();
    });
  });
});
...
describe("searchRegion", () => {
  ...
  test("no match", () => {
    expect(searchRegion("Wallonie")).toBeNull();
  });
});
